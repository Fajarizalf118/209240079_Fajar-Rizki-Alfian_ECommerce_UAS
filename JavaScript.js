...
function filterProduk() {
  ...
  produkList.forEach(produk => {
    ...
    if (title.includes(input) || description.includes(input)) {
      produk.style.display = "flex"; // Gunakan flex agar kembali normal
    } else {
...
function checkout() {
  ...
  // Kode event listener untuk tombol bayar HARUS ada di dalam fungsi checkout
  ...
  newPayButton.addEventListener('click', function() {
    ...
    cart.length = 0; 
    closePaymentModal();
    bukaKeranjang(); // Update tampilan keranjang
  });
} // <-- Kurung kurawal penutup `checkout` SEHARUSNYA DI SINI
...
function slideKategori(direction) {
  ...
  const cardWidth = track.querySelector('.kategori-card').offsetWidth + 32; // 32px gap
